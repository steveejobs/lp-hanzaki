...
import type { HanzakiTestimonial } from "@/src/data/hanzaki-reviews";

type InstagramTestimonialsMarqueeProps = {
  reviews: HanzakiTestimonial[];
};

function ReviewCard({ review }: { review: HanzakiTestimonial }) {
  return (
    <article className="h-[136px] w-[280px] shrink-0 rounded-[22px] border border-black/10 bg-white p-4 shadow-[0_10px_24px_rgba(16,16,16,0.055)]">
      <span className="block text-sm font-black leading-none text-[var(--hanzaki-red)]">
        ★★★★★
      </span>
      ...
      <div className="mt-3 flex items-center justify-between gap-3 text-xs font-black text-neutral-950">
        ...
        <span className="rounded-full bg-[#fff4ec] px-2.5 py-1 text-[var(--hanzaki-red)]">
          {review.source}
        </span>
      </div>
    </article>
    ...
    <section ref={sectionRef} className="ig-rise mt-6 overflow-hidden">
      <div className="px-1">
        ...
        <p className="mt-1 text-sm font-bold leading-6 text-neutral-600">
          Depoimentos editoriais identificados para composição visual.
        </p>
      </div>
